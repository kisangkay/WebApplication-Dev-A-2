...
class ReviewsController extends Controller
{
     ...
     */
    public function index(string $cid, $assesst_id,  $sid)
    {
        $reviewer_all_data = User::where('id', $sid)
            ->first();

        $assessment_data = Assessment::where('id', $assesst_id)//assessment model and student id to show score for userid
            ->first();

        $assessment_score = AssessmentScore::where('user_id', $sid)
        ...
        $submittedreviews = Review::where('reviewer_user_id', $sid)
        ->where('assessment_id', $assesst_id)//also assessmentid has to be associated to this assessment
        ->get();

        $receivedreviews = Review::where('reviewee_user_id', $sid)
            ->where('assessment_id', $assesst_id)//also assessmentid has to be associated to this assessment
            ->get();
