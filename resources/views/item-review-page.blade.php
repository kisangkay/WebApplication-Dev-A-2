...
@section('content')
    ...
    @if(session('reviewdeleted'))
        <div class=" h6 alert alert-primary text-light alert fade show border-bottom text-center" role="alert">
    ...
    <div class="container py-5">
        <div class="h2 px-2 text-center">Reviews for {{$items[0]->bicycle_name}}
            <h6 class=" text-info mb-4">Posted on: {{$items[0]->bicycle_date_posted}} By
                user_id: {{$items[0]->bicycle_poster_user_id}}</h6>
            <div class="d-flex justify-content-center">
                <img class="card card-cover rounded-4 w-25 m-3"
                     src="{{asset('storage/').'/'.$bikeimagepath[0]->bicycle_image}}">
            </div>
            <h4 class="mt-2 fw-bold text-center text-info">{{$items[0]->bicycle_name}}</h4>
            {{--            @dd($items)--}}
            <h5 class="fw-bold text-info mb-4">Manufacturer: {{$items[0]->manufacturer_name}}</h5>
