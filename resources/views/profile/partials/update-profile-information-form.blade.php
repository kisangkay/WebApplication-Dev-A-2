<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            <x-input-label for="user_number" :value="__('Usernumber')" class="text-white" />
        ...
        <div>
            ...
            <x-text-input id="fullname" name="fullname" type="text" class="mt-1 block w-full bg-gray-700 text-dark" :value="old('fullname', $user->fullname)" required autofocus autocomplete="fullname" />
            <x-input-error class="mt-2 text-red-500" :messages="$errors->get('fullname')" />
        </div>

        <div>
            <x-input-label for="email" :value="__('Email')" class="text-white" />
            <x-text-input id="email" name="email" type="email" class="mt-1 block w-full bg-gray-700 text-dark" :value="old('email', $user->email)" required autocomplete="username" />
